...
class Trainer:
	def __init__(self, conf_file, embeddings, vocabulary):
		'''
	...
	def configure(self):
		...
		if(self.conf_data['type'] == "cnn"):
			...
			#Creamos modelo
			self.model = CNN_Model(self.embeddings, self.vocabulary, train_path, test_path, max_length, batch_size, filtro_1, filtro_2, kernel_1, kernel_2, dropout_1, dropout_2)
		elif(self.conf_data['type'] == "bilstm"):
			num_units = int(self.conf_data['units_lstm'])

			self.model = LSTM_Model(self.embeddings, self.vocabulary, train_path, test_path, max_length, batch_size, num_units)
			...
			self.model = Combined_Model(self.embeddings, self.vocabulary, train_path, test_path, max_length, batch_size, filtro_1, filtro_2, kernel_1, kernel_2, dropout_1, dropout_2, num_units)
	...
	def getTransitionParams(self):
		return self.model.getTransitionParams()
